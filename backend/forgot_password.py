...
def verify_email(body: VerifyEmailRequest):
    try:
        ...
        logger.error("DynamoDB get employee failed for %s: %s", body.work_email, e, exc_info=True)
...
def send_reset(body: SendResetRequest, request: Request):
    try:
        ...
        logger.error("DyanmoDB get employee failed for %s: %s", body.work_email, e, exc_info=True)
    ...
    if not profile or not profile.get("onboarding_complete"):
        raise HTTPException(status_code=400, detail="No employee found matching the provided details")
    
    stored_personal_email = profile.get("personal_email", "")
    if body.personal_email.lower() != stored_personal_email.lower():
        raise HTTPException(status_code=400, detail="No employee found matching the provided details")
    
    stored_nric_hash = profile.get("nric_last4_hash", "")
    if not stored_nric_hash or not bcrypt.checkpw(body.nric_last4.upper().encode(), stored_nric_hash.encode()):
        raise HTTPException(status_code=400, detail="No employee found matching the provided details")
    ...
    try:
        ...
        logger.error("DynamoDB update employee reset token failed for %s: %s", body.work_email, e, exc_info=True)
        raise HTTPException(status_code=503, detail="Failed to process request")
    ...
    try:
        ...
        logger.error("Resend email failed for %s: %s", body.work_email, e, exc_info=True)
        try:
            clear_employee_reset_token(email=body.work_email)
        except ClientError:
            logger.error("Failed to clear reset token for %s after Resend failure", body.work_email)
...
def reset_password(body: ResetPasswordRequest):
    ...
    try:
        ...
        logger.error("Cognito admin set user password failed for %s: %s", body.work_email, e, exc_info=True)
        raise HTTPException(status_code=502, detail="Failed to reset password")
    
    try:
        clear_employee_reset_token(email=body.work_email)
    except ClientError as e:
        logger.error("DynamoDB clear employee reset token failed for %s: %s", body.work_email, e, exc_info=True)
        logger.warning("Reset token for %s not cleared - will expired naturally %s", body.work_email, token_expiry)
