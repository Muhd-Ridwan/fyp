...
def register_employee(
    ...
    try:
        _cognito.admin_create_user(
            ...
            UserAttributes=[{"Name": "email", "Value": body.personal_email}],
    ...
    try:
        ...
        try:
            ...
            logger.error("Rollback failed - DynamoDB employee %s left orphaned", body.email)
        try:
            _cognito.admin_delete_user(UserPoolId=config.COGNITO_USER_POOL_ID, Username=body.email)
        except ClientError:
            logger.error("Rollback failed - COgnito user %s left orphaned after Resend failure", body.email)
