...
async def upload_document(
    ...
    try:
        ...
        logger.error("DynamoDB create document failed for %s: %s", file_id, e, exc_info=True)
        try:
...
def download_document(
    ...
    try:
        ...
        logger.error("S3 presign failed for %s: %s", document["s3_key"], e, exc_info=True)
...
def rename_document(
    ...
    try:
        ...
        logger.error("DynamoDB rename_document failed for %s: %s", file_id, e, exc_info=True)
...
def delete_document(
    ...
    try:
        ...
        logger.error("S3 delete failed for %s: %s", document["s3_key"], e, exc_info=True)
    ...
    try:
        ...
        logger.error("DyanmoDB delete_document failed for %s: %s", file_id, e, exc_info=True)
